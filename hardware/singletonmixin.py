"""
Converted with 2to3 and some modifications in error handling
...
def _createSingletonInstance(cls, lstArgs, dctKwArgs):
    ...
    try:
        ...
        try:
            ...
            msg = 'If the singleton requires __init__ args, supply them on first call to getInstance().'
            if str(e).find('__init__() takes') != -1:
                raise SingletonException(msg)
            elif str(e).find('__init__() missing') != -1:
                raise SingletonException(msg)
            else:
                raise
