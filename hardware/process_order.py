...
class Serve(AdvanceOrder):
    def _wait_for_glass_removed(self, glass_weight):
        return self.interface.cell_wait_for_weight_total(
    ...
    def advance_order(self):
        ...
        for dose in doses:
            ...
            if not quantity_is_reached:
                ...
                if self.interface.state != 2:
                    logger.info('Interface has moved on from this order')
                else:
                    # assume problem with the pump
                    logger.info(
